    ...
    <div className="space-y-6 py-4">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 px-2">
        <div className="flex items-center gap-4">
          ...
          <div><h2 className="text-2xl font-black text-[#372C2E]">ข้อมูลลูกค้า</h2><p className="text-xs text-[#A1887F] font-bold">จัดการโปรไฟล์และการดูแลน้องแมว</p></div>
        </div>
        <div className="flex gap-2 w-full md:w-auto">
          <input type="text" placeholder="ค้นหาชื่อ/เบอร์..." className="pl-4 pr-4 py-2.5 bg-white border border-[#efebe9] rounded-xl outline-none focus:border-[#885E43] font-bold text-sm w-full md:w-64" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          <button onClick={() => { setModalMode('add'); setEditingCustomer({ name: '', phone: '', source: 'Line', source_id: '', cameraId: '-', eating_habit: '', note: '', image: '' }); setIsModalOpen(true); }} className="bg-[#885E43] text-white px-4 py-2.5 rounded-xl font-bold flex items-center gap-2 hover:bg-[#5D4037] shadow-lg text-sm"><Plus size={18} /> เพิ่มลูกค้า</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filtered.map((customer, idx) => (
          <div key={idx} className="bg-white rounded-[2rem] p-5 border border-[#efebe9] shadow-sm hover:shadow-md transition-all relative overflow-hidden flex flex-col">
            ...
            {/* ข้อมูลการกินและหมายเหตุในการ์ด */}
            <div className="mt-4 p-3 bg-[#FDFBFA] rounded-xl border border-[#efebe9]/50 space-y-2">
              <div className="flex items-start gap-2">
                ...
                <p className="text-[10px] text-[#372C2E] leading-relaxed"><span className="font-bold text-[#885E43]">การกิน:</span> {customer.eating_habit || '-'}</p>
              </div>
              <div className="flex items-start gap-2">
                <FileText size={12} className="text-[#DE9E48] mt-0.5 flex-shrink-0" />
                <p className="text-[10px] text-[#372C2E] leading-relaxed"><span className="font-bold text-[#885E43]">หมายเหตุ:</span> {customer.note || '-'}</p>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white w-full max-w-md my-8 rounded-[2rem] overflow-hidden shadow-2xl">
            ...
            <div className="p-6 space-y-4">
              ...
              {/* ช่องกรอกข้อมูลใหม่ */}
              <div><label className="text-[10px] font-bold text-[#A1887F] uppercase">น้องกินยังไง / สูตรอาหาร</label>
                <textarea className="w-full p-2.5 bg-[#FDFBFA] border rounded-xl text-sm" rows="2" placeholder="เช่น กินอาหารเม็ดเช้า-เย็น, ผสมเปียกตอนค่ำ..." value={editingCustomer.eating_habit} onChange={e => setEditingCustomer({...editingCustomer, eating_habit: e.target.value})}></textarea>
              </div>

              <div><label className="text-[10px] font-bold text-[#A1887F] uppercase">หมายเหตุเพิ่มเติม</label>
                <textarea className="w-full p-2.5 bg-[#FDFBFA] border rounded-xl text-sm" rows="2" placeholder="เช่น กลัวเสียงดัง, ชอบให้เกาพุง..." value={editingCustomer.note} onChange={e => setEditingCustomer({...editingCustomer, note: e.target.value})}></textarea>
              </div>
