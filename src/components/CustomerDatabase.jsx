...
import { 
  ...
  Calendar, DoorOpen, Clock, History, X 
} from 'lucide-react';
...
export default function CustomerDatabase() {
  ...
  const handleDelete = async (customerName) => {
    if (window.confirm(`คุณแน่ใจหรือไม่ว่าต้องการลบข้อมูลของ "คุณ ${customerName}"? การลบจะลบประวัติการจองทั้งหมดที่ชื่อตรงกัน`)) {
      try {
        ...
        {currentData.map((customer, idx) => (
          ...
          >
                ...
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex gap-1">
                    <button 
                      ...
                      className="p-2 text-[#885E43] hover:bg-[#FDF8F5] rounded-lg transition-colors"
                    >
                      <Edit3 size={18}/>
                    </button>
                    <button 
            ...
            <div className="mt-4 space-y-2">
              ...
              <div className="bg-[#FDFBFA] p-3 rounded-xl border border-[#efebe9]/50">
                <div className="flex items-center gap-2 text-[#885E43] font-bold text-[10px] mb-1 uppercase"><FileText size={12}/> หมายเหตุถาวร</div>
          ...
          <div className="bg-white w-full max-w-3xl h-[85vh] md:h-auto md:max-h-[90vh] rounded-t-[2.5rem] md:rounded-[2.5rem] flex flex-col shadow-2xl overflow-hidden">
            <div className="bg-[#372C2E] p-6 text-white flex justify-between items-center shrink-0">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-bold text-xl leading-tight">{historyModal.name}</h3>
                  <p className="text-xs text-[#DE9E48] font-bold">ประวัติการเข้าพักทั้งหมด</p>
                </div>
              </div>
            ...
            <div className="p-6 overflow-y-auto bg-[#FDFBFA] space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white p-4 rounded-3xl border border-[#efebe9] shadow-sm">
                  <div className="flex items-center gap-2 text-[#885E43] font-bold text-sm mb-2"><Utensils size={16}/> ข้อมูลการกิน</div>
                  <p className="text-sm text-[#372C2E] font-medium leading-relaxed">{historyModal.eating_habit || '-'}</p>
                </div>
                <div className="bg-white p-4 rounded-3xl border border-[#efebe9] shadow-sm">
                  <div className="flex items-center gap-2 text-[#DE9E48] font-bold text-sm mb-2"><FileText size={16}/> หมายเหตุถาวร</div>
                  <p className="text-sm text-[#372C2E] font-medium leading-relaxed">{historyModal.note || '-'}</p>
                </div>
              </div>

              <div className="space-y-4">
                <div className="flex items-center gap-2 font-black text-[#372C2E] text-lg px-1">
                  <Clock size={20} className="text-[#885E43]"/> รายการเข้าพักรายครั้ง
                </div>
                {historyModal.history.map((h, i) => (
                  <div key={i} className="bg-white p-5 rounded-[2rem] border border-[#efebe9] shadow-sm hover:shadow-md transition-shadow">
                    <div className="grid grid-cols-4 gap-4 items-center">
                      <div className="flex flex-col">
                        <span className="text-[10px] font-bold text-[#A1887F] mb-1 uppercase">วันที่เข้าพัก</span>
                        <div className="flex items-center gap-2 text-[12px] font-black text-[#372C2E]">
                          <Calendar size={14} className="text-[#885E43] shrink-0" />
                          <span>{h.start_date} <span className="text-[#885E43]">ถึง</span> {h.end_date}</span>
                        </div>
                      </div>
                      <div className="flex flex-col">
                        <span className="text-[10px] font-bold text-[#A1887F] mb-1 uppercase">ประเภทห้อง</span>
                        <div className="text-blue-600 font-bold text-[13px] flex items-center gap-1.5"><DoorOpen size={14}/> {h.room_type}</div>
                      </div>
                      <div className="flex flex-col">
                        <span className="text-[10px] font-bold text-[#A1887F] mb-1 uppercase">น้องแมว</span>
                        <div className="text-[#DE9E48] font-bold text-[13px] flex items-center gap-1.5 truncate"><Cat size={14}/> {h.cat_names}</div>
                      </div>
                      <div className="flex flex-col text-right">
                        <span className="text-[10px] font-bold text-[#A1887F] mb-1 uppercase">ค่าที่พัก</span>
          ...
          <div className="bg-white w-full max-w-md rounded-[2.5rem] overflow-hidden shadow-2xl">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="space-y-3">
                ...
                <button onClick={handleSave} className="w-full py-4 bg-[#885E43] text-white rounded-2xl font-black shadow-lg">บันทึกข้อมูล</button>
              </div>
            </div>
          </div>
        </div>
