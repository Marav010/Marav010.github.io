...
import {
  ...
  CheckCircle2, AlertCircle, XCircle, Wallet, Check, User, ShoppingBag
} from 'lucide-react';
...
export default function BookingForm({ onSaved, initialDate }) {
  ...
  const [formData, setFormData] = useState({
    ...
    extra_items: [] // เพิ่ม state สำหรับของกิน/ของใช้เพิ่มเติม
  });

  // --- ฟังก์ชันจัดการรายการเพิ่มเติม ---
  const addExtraItem = () => {
    setFormData({
      ...formData,
      extra_items: [...formData.extra_items, { item_name: '', price: 0 }]
    });
  };
  ...
  const bookingSummary = useMemo(() => {
    ...
    formData.cats.forEach(cat => {
      ...
      totalRooms += pricePerNight * validNights;
    ...
    // รวมยอดของกิน/ของใช้เพิ่มเติม
    const extraTotal = formData.extra_items.reduce((sum, item) => sum + (item.price || 0), 0);
    const total = totalRooms + extraTotal;

    return { nights: validNights, total, depositValue, extraTotal };
  }, [formData]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // สร้าง Note อัตโนมัติจากรายการสินค้าเพิ่มเติมเพื่อเก็บลง Database
      const extraNotes = formData.extra_items
      ...
      const bookingsToInsert = formData.cats.map(cat => {
        ...
        return {
          ...
          deposit: catDeposit,
          note: extraNotes || '-' // บันทึกรายการของที่ซื้อเพิ่มลงในช่อง note
        };
      });
    ...
    <div className="max-w-2xl mx-auto py-4 md:py-8 animate-in slide-in-from-bottom-4 duration-500 font-sans px-4">
      <div className="bg-white rounded-[2.5rem] p-6 md:p-10 shadow-xl border border-[#efebe9]">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-end px-1">
              <label className="text-xs font-black text-[#885E43] uppercase tracking-widest flex items-center gap-2">
                <ShoppingBag size={14} /> รายการเพิ่มเติม (อาหาร/ทราย/ขนม)
              </label>
              <button type="button" onClick={addExtraItem} className="text-[10px] bg-[#DE9E48] text-white px-4 py-2 rounded-xl font-bold flex items-center gap-2 hover:bg-[#c68a39] active:scale-95 transition-all">
                <Plus size={14} /> เพิ่มรายการ
              </button>
            </div>
            {formData.extra_items.map((item, index) => (
              <div key={index} className="grid grid-cols-12 gap-3 animate-in fade-in slide-in-from-left-2">
                <div className="col-span-7">
                  <input
                    placeholder="รายการ (เช่น ขนมแมวเลีย)"
                    className="w-full p-3 bg-[#FDFBFA] rounded-xl border-2 border-[#efebe9] focus:border-[#DE9E48] outline-none text-sm font-bold text-[#372C2E]"
                    value={item.item_name}
                    onChange={e => updateExtraItem(index, 'item_name', e.target.value)}
                  />
                </div>
                ...
                <div className="col-span-1 flex items-center">
                  <button type="button" onClick={() => removeExtraItem(index)} className="text-red-400 hover:text-red-600 transition-colors">
                    <Trash2 size={18} />
                  </button>
                </div>
              </div>
          ...
          <div className="bg-[#372C2E] rounded-[2.5rem] p-8 text-white shadow-2xl border border-[#5d4037] relative overflow-hidden">
            <div className="relative z-10 space-y-4">
              <div className="flex justify-between items-center opacity-60">
                <span className="text-xs font-bold uppercase tracking-widest">ยอดรวมทั้งหมด ({bookingSummary.nights} คืน + รายการเพิ่ม)</span>
